...
module.exports = class Generate {
    ...
    generateResponse() {
        ...
        while (!doneGeneration) {
            ...
            while (!finishedMessage) {
                ...
                this.utils.logDebug("");
                this.utils.logDebug("***** Message: \"" + this.compileMessage(tweetWords) + "\" *****");

                // If the sentence naturally comes to an end
                if (this.utils.endsWithPunc(this.compileMessage(tweetWords))) {
                    this.utils.logDebug("    * Punctuation found");

                    // If adequite length, exit from generation
                    if (length >= 4 && this.compileMessage(tweetWords).length < 139) {
                    ...
                    // Otherwise pop last word & mark as visited.
                        ...
                        if (tweetWords >= 3) {
                            ...
                            this.backtrace(tweetWords, firstWords);
                            tweetWords.pop();
                        } else {
                ...
                // If there is a entry for a new word, add it to the message
                    ...
                    for (let i = 0; !hasFoundNextWord && i < numOfWords; i++) {
                        ...
                        if (!wordEntry.backtraced) {
                            ...
                            tweetWords.push(this.wordDict[key][nextWord].word);
        ...
        result = this.compileMessage(tweetWords);
     ...
     *   tweetWords - Current array of words for the new tweet.
     *   firstWords - Array of first words.
     */
    backtrace(tweetWords, firstWords) {
        let word, key;
        const length = tweetWords.length;
        ...
        // If only 2 words in message, remove from first list
            ...
            const index = firstWords.indexOf(key);
            firstWords.splice(index, 1);
        }
    }
