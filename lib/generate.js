...
module.exports = class Generate {
    ...
    generateResponse() {
        ...
        while (!doneGeneration) {
            ...
            this.tweetMessage = [];
            this.tweetMessage.push(...(this.first[firstWord].split(" ")));

            // Construct the rest of the tweetMessage
            finishedMessage = false;
            while (!finishedMessage) {
                const length = this.tweetMessage.length;
                const key = this.tweetMessage[length - 2] + " " + this.tweetMessage[length - 1];
                ...
                if (this.utils.endsWithPunc(this.compileMessage())) {
                    ...
                    // Otherwise pop last word & mark as visited.
                        ...
                        if (this.tweetMessage >= 3) {
                            this.utils.logDebug("    * Popping last word");
                            this.backtrace();
                            this.tweetMessage.pop();
                        } else {
                ...
                // If there is a entry for a new word, add it to the message
                    ...
                    for (let i = 0; !hasFoundNextWord && i < numOfWords; i++) {
                        ...
                        if (!wordEntry.backtraced) {
                            ...
                            this.tweetMessage.push(this.wordDict[key][nextWord].word);
    ...
    backtrace() {
        ...
        if (length >= 3) {
            key = this.tweetMessage[length - 3] + " " + this.tweetMessage[length - 2];
            word = this.tweetMessage[length - 1];

            this.getObjectByWord(this.wordDict[key], word).backtraced = true;
        // If only 2 words in message, remove from first list
        } else {
            key = this.tweetMessage[0] + " " + this.tweetMessage[1];

            const index = this.first.indexOf(key);
    ...
    compileMessage() {
        ...
        for (let i = 0; i < this.tweetMessage.length; i++) {
            result += this.tweetMessage[i];

            if (i < this.tweetMessage.length - 1) {
                result += " ";
            }
        }
