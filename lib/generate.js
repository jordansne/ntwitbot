...
module.exports = class Generate {
    ...
    generateTweet(wordDB) {
        ...
        while (!done) {
            ...
            while (true) {
                ...
                const currentMessage = this.compileMessage(wordStack);

                // If the sentence naturally comes to an end with punctuation
                if (this.utils.endsWithPunc(wordStack[numOfWords - 1])) {
                    if (numOfWords >= this.MIN_WORDS) {
                        if (currentMessage.length <= this.MAX_CHARS) {
                            done = true;
                            break;
                        } else {
                ...
                // If there is an entry for the next word
                if (wordDB.hasOwnProperty(key)) {
                    const nextWord = this.getRandomWord(wordDB, key);
    ...
    popWord(wordStack, wordDB) {
        ...
        if (numOfWords >= 3) {
            ...
            this.getWordObject(wordDB[key], word).beenPopped = true;
            wordStack.pop();
        } else {
            wordStack.splice(0);
        }
