...
module.exports = class Main {
    ...
    constructor() {
        ...
        // Catch any start up errors
            ...
            this.utils.logError("FATAL:     Error: " + error.name + error.message);
            this.utils.logError("FATAL:     Stack: " + error.stack);
    ...
    handleTweets() {
        return this.retriever.retrieveTweets().then((newTweets) => {
            if (newTweets === null) {
                this.utils.log("Retrieved tweets: No new tweets found");
                return Promise.reject();
            }

            this.utils.log("Retrieved tweets: " + newTweets.length + " new tweets found");
            return this.dataHandler.saveTweetData(this.processor.processTweets(newTweets));
        }, (error) => {
            this.utils.logError("Failed to retrieve new tweets");
            this.utils.logError("    Error: " + error.name + error.message);
            this.utils.logError("    Stack: " + error.stack);
    ...
    sendTweet(replyToID, replyTo) {
        return this.dataHandler.readTweetData().then((data) => {
            ...
            return this.twitterHandler.postTweet(tweet, replyToID, replyTo).then(() => {
