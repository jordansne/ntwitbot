...
module.exports = class Event {
    ...
    constructor(twitterHandler, dataHandler, generator, utils) {
        ...
        this.utils = utils;
        this.processor = new Process(dataHandler, utils);
    ...
    checkTrackedTweets() {
        ...
        return new Promise((resolve, reject) => {
            this.twitterHandler.retrieveFollowing(this.dataHandler.ownID).then((following) => {
                ...
                Promise.all(tweetRetrievals).then(() => {
                    if (this.updateTweets) {
                        ...
                        // Resolve with the bot data when done processing tweets
                        this.processor.processTweets(tweetsToProcess).then(() => {
                            resolve(this.botStatus);
                        });
     ...
     */
    handleMention(tweet) {
        this.twitterHandler.postTweet(this.generator.generateResponse(), tweet.id_str, tweet.user.screen_name);
    }
    ...
    retrieveTweetsSince(userData, tweetsToProcess, resolve, tweetsRetrievedTotal) {
        this.twitterHandler.retrieveTweets(userData).then((tweets) => {
