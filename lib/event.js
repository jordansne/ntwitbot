...
module.exports = class Event {
     ...
     *   done                 - Function: Function to call when done retrieving tweets.
     *   tweetsRetrievedTotal - Int:      Used by recursive call, do not specify in initial call.
     */
    retrieveTweetsSince(userData, tweetsToProcess, done, tweetsRetrievedTotal) {
        this.twitterPkg.get('statuses/user_timeline', userData, (error, tweets, response) => {
            ...
            if (tweetsRetrieved > 0) {
                ...
                this.retrieveTweetsSince(userData, tweetsToProcess, done, tweetsRetrievedTotal);
     ...
     *   done            - Function: Function to call when done retrieving tweets.
     */
    retrieveTweetsNew(userData, tweetsToProcess, numOfTweets, done) {
        this.twitterPkg.get('statuses/user_timeline', userData, (error, tweets, response) => {
            if (error) {
                this.utils.logError("Failed to retrieve tweets from user: " + userData.user_id);
            ...
            if (numOfTweets > 0) {
                ...
                this.utils.log("    Retrieved " + tweetsRetrievedTotal + "/" + this.TWEETS_TO_TRACK + " tweets");
                this.retrieveTweetsNew(userData, tweetsToProcess, numOfTweets, done);
            } else {
