...
module.exports = class Event {
    ...
    checkMentions() {
        ...
        this.twitterPkg.get('statuses/mentions_timeline', mentionData, (error, mentions, response) => {
            if (error) {
                console.error("[ERROR] Failed to retrieve mentions");
                console.error("[ERROR]     Error message: %s", error[0].message);
                return;
            }

            console.log("[INFO] %d new mentions found", mentions.length);
    ...
    checkTrackedTweets() {
        ...
        this.twitterPkg.get('friends/ids', data, (error, result, response) => {
            if (error) {
                console.error("[ERROR] Failed to retrieve following list of bot");
                console.error("[ERROR]     Error message: %s", error[0].message);
                return;
            }
            ...
            for (let userID in this.trackedUsers) {
                ...
                this.twitterPkg.get('statuses/user_timeline', userData, (error, tweets, response) => {
                    if (error) {
                        console.error("[ERROR] Failed to retrieve tweets from user: %d", userData.userID);
                        console.error("[ERROR]     Error message: %s", error[0].message);
                        return;
                    }

                    console.log("[INFO] Found %d new tweets from user: %s", tweets.length, tweets[0].user.id);
