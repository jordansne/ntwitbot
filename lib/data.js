...
module.exports = class Data {
    ...
    processTrackedTweets(tweets) {
        ...
        console.log("[INFO] Update: Processing %d new tweets", tweets.length);
        ...
        fs.readFile(this.DATABASE_DIR + '/db.json', (error, data) => {
            if (error) {
                if (error.code === "ENOENT") {
                    ...
                    console.error("[ERROR] Failed to open database file (" + this.DATABASE_DIR + "/db.json)");
    ...
    inputToEmpty(data) {
        ...
        fs.open(this.DATABASE_DIR + '/db.json', 'w', (error, fd) => {
            if (error) {
                console.error("[ERROR] Failed create new database file (" + this.DATABASE_DIR + "/db.json')");
            ...
            fs.writeFile(this.DATABASE_DIR + '/db.json', JSON.stringify(data), (error, data) => {
                if (error) {
                    console.error("[ERROR] Failed to write new data to database file (" + this.DATABASE_DIR + "/db.json')");
    ...
    inputToExisting(data, newData) {
        ...
        fs.writeFile(this.DATABASE_DIR + '/db.json', JSON.stringify(data), (error, data) => {
            if (error) {
                console.error("[ERROR] Failed to append new data to database file (" + this.DATABASE_DIR + "/db.json')");
                console.error("[ERROR]     Error code: ", error.code);
                return;
            }
    ...
    saveStatus(botStatus) {
        fs.writeFile(this.DATABASE_DIR + '/status.json', JSON.stringify(botStatus), (error, botStatus) => {
            if (error) {
                console.error("[ERROR] Failed to save bot status to file (" + this.DATABASE_DIR + "/status.json')");
                console.error("[ERROR]     Error code: ", error.code);
                return;
            }

            console.log("[INFO] Saved bot status to file");
        });
    }

    /* Gets the botStatus from the file.
     *
     *   Returns - botStatus: Object containing current bot status.
     */
    readStatus() {
        let botStatus;
        ...
        try {
            ...
            if (error.code == 'ENOENT') {
                ...
                console.error("[ERROR] FATAL: Failed to read botStatus file (" + this.DATABASE_DIR + "/status.json)");
        ...
        console.log("[INFO] Read bot status from file");
