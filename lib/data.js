...
module.exports = class Data {
     ...
     *   Returns - Object: Object containing current bot state.
     */
    readState() {
        let state;

        try {
            state = fs.readFileSync(this.DATABASE_DIR + '/state.json', 'utf8');
        } catch (error) {
            if (error.code == 'ENOENT') {
                return null;
            } else {
                this.utils.logError("I/O: FATAL: Failed to read state file (" + this.DATABASE_DIR + "/state.json)");
